...
class App(object):
    ...
    def checkForEnemyBullets(self):
        ...
        for bullet in redBulletsGroup:
            if pygame.sprite.collide_rect(bullet, self.player):
                if Player.player.color == Colors.GREEN:
                    Player.player.color = Colors.YELLOW
                elif Player.player.color == Colors.YELLOW:
                    Player.player.color = Colors.RED
                elif Player.player.color == Colors.RED:
                    self.gameOver = True
                    self.gameOverTime = pygame.time.get_ticks()
                bullet.kill()
    ...
    def shootEnemyBullet(self, rect):
        if (pygame.time.get_ticks() - self.enemyBulletTimer) > Bullet.BULLETOFFSET:
            Bullet.Bullet.add(Bullet.Bullet(rect, Colors.RED, 1, 5))
